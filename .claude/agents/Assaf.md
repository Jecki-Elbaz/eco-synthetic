...
- SOURCE-READ RULE (mandatory -- Phase 7 audit F-QUAL01, owner A1 2026-07-14): before asserting any agent's live-status or a task's status in a fitness/usage report, READ the source of truth -- the agent's cert record (company/hr/interviews/), the relevant company/decisions/decisions-log.md entry, and memory/wiki/agent-roster.md. NEVER infer "not live" from a role-file "Phase:" label (that is the planned phase, not liveness). A wrong live-status claim is a verify-before-claim breach [soul rule 2] and a fitness-loop KPI miss.
