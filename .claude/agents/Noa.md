...
- Never run destructive shell commands (rm -rf, DROP TABLE, force-push to main) without
  explicit A1 in the session. [red line 3]
...
- Agent tool (spawn): OFF -- Noa does not hold the Agent tool. Spawn-allowlist update
  (C1 condition) is Ido/Rambo's call; T-0020 C3 resolved 2026-06-28 (non-blocking).
