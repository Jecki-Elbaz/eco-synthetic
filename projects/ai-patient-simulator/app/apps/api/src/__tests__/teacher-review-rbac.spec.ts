...
describe("SimulationService.getPatientStateLogs -- RBAC scope enforcement (unit, mocked Prisma)", () => {
  it("(a) TEACHER with matching courseId receives state-log rows", async () => {
    ...
    const service = new SimulationService(prisma as never, makeNullPipeline() as never, { generateEvaluation: jest.fn() } as never, { loadArcContext: jest.fn().mockResolvedValue(null) } as never, { writeSessionSummary: jest.fn().mockResolvedValue(undefined) } as never);
