...
describe("SimulationService -- softWarnAnnotation", () => {
  it("softWarnAnnotation is null when softWarnTriggered is false", async () => {
    ...
    const service = new SimulationService(prisma as never, pipeline as never, { generateEvaluation: jest.fn() } as never, { loadArcContext: jest.fn().mockResolvedValue(null) } as never, { writeSessionSummary: jest.fn().mockResolvedValue(undefined) } as never);
