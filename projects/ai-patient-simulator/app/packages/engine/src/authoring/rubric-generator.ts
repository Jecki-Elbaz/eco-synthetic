...
export const COMPETENCY_EXTERNAL_KEYS: Record<string, string> = {
...
interface CriterionTemplate {
  ...
  labelsHe: string;
  labelsEn: string;
  weight: number;           // proportion of total summative weight (risk criterion = 0)
  maxScore: number;
  scoringAnchors: Array<{ score: number; label: string }>;
...
const BASE_CRITERIA: CriterionTemplate[] = [
  {
    labelKey: "empathy",
    labelsHe: "אמפתיה",
    labelsEn: "Empathy",
 ...
 */
export function generateRubricCriteria(fields: Pick<BuilderFields, "challengeLevel" | "riskLevel">): GeneratedRubric {
  // Clone base criteria
  const criteria: RubricCriterionDraft[] = BASE_CRITERIA.map((c) => ({
    labelKey: c.labelKey,
    labels: { he: c.labelsHe, en: c.labelsEn },
    weight: c.weight,
    maxScore: c.maxScore,
    scoringAnchors: c.scoringAnchors,
    competencyId: COMPETENCY_EXTERNAL_KEYS[c.competencyKey],
