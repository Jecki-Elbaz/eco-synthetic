...
const CHALLENGE_LEVEL_DESCRIPTORS: Record<number, string> = {
 ...
 */
export function composePersonaPrompt(fields: BuilderFields): string {
  const {
    clinicalModel,
    studentLevel,
    primarySkill,
  ...
  const levelLabel =
    STUDENT_LEVEL_LABELS[studentLevel] ?? studentLevel;

  const challengeDesc =
    CHALLENGE_LEVEL_DESCRIPTORS[challengeLevel] ??
    `challenge level ${challengeLevel}`;

  const riskDesc =
    RISK_LEVEL_DESCRIPTORS[riskLevel] ?? `risk level: ${riskLevel}`;
