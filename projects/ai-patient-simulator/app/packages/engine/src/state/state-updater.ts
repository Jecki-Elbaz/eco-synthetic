...
import { DEFAULT_DELTA_CAP_CONFIG } from "./delta-cap.config.js";
...
function applyDelta(current: number, delta: number, maxDelta: number): number {
...
export interface StateUpdateResult {
  ...
  firedTriggerConditions: string[];  // trigger condition expressions that evaluated true
}

export class StateUpdater {
  private readonly config: DeltaCapConfig;

  constructor(config: DeltaCapConfig = DEFAULT_DELTA_CAP_CONFIG) {
    this.config = config;
  }
  ...
  update(
    ...
    const canIncreaseTrustOpenness =
      analyser.empathy >= cfg.trustOpennessIncreaseEmpathyThreshold;
    const canDecreaseAvoidanceDefensiveness =
      analyser.pressure <= cfg.avoidanceDefensivenessDecreaseMaxPressure;
    ...
    if (trustDelta > 0 && !canIncreaseTrustOpenness) trustDelta = 0;
    ...
    if (opennessDelta > 0 && !canIncreaseTrustOpenness) opennessDelta = 0;
    ...
    if (avoidanceDelta < 0 && !canDecreaseAvoidanceDefensiveness) avoidanceDelta = 0;
    ...
    if (defensivenessDelta < 0 && !canDecreaseAvoidanceDefensiveness) defensivenessDelta = 0;
    ...
    const firedTriggerConditions: string[] = [];
