...
exports.ContextBuilder = exports.selectWindowByTokenBudget = exports.estimateTokens = void 0;
// ---------------------------------------------------------------------------
 ...
 */
function selectWindowByTokenBudget(messages, tokenBudget) {
    let used = 0;
    const selected = [];
...
exports.selectWindowByTokenBudget = selectWindowByTokenBudget;
...
class ContextBuilder {
    build(input) {
        ...
        const windowedMessages = input.contextTokenBudget !== undefined
            ? selectWindowByTokenBudget(input.recentMessages, input.contextTokenBudget)
            : input.recentMessages;
        // --- System prompt: persona + challenge instructions ---
        const challengeInstruction = CHALLENGE_INSTRUCTIONS[input.challengeLevel] ?? CHALLENGE_INSTRUCTIONS[3];
        const systemPrompt = [
        ...
        if (input.contextSummary) {
            messages.push({
                role: "system",
                content: "## EARLIER SESSION SUMMARY (turns before current window)\n" +
     ...
     */
    buildGuardPrompt(proposedResponse, groundTruth, turnNumber) {
        const systemPrompt = [
