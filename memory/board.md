...
| T-0021 | Ido | open | Validate and lock the auto-pull sync mechanism: (1) confirm UserPromptSubmit hook in .claude/settings.json fires correctly and pulls master on every Eco session start; (2) assess whether the 2h scheduled wake-up also needs a pull trigger or if session-start is sufficient; (3) evaluate edge cases (conflicts, network failure, ff-only rejection) and harden the command if needed; (4) document the final mechanism in company/processes/ or equivalent; (5) escalate any concern to Eco. Urgent -- this blocks Eco from seeing current repo state in Telegram sessions. | P1 | urgent | Created 2026-06-14; hook written but settings.json edit blocked by auto-mode classifier -- owner must apply manually (see board notes) |
