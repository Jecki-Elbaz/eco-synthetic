...
| T-0019 | Eco | open | Competency testing for all 10 P1 agent drafts: Eco is direct manager for Ido, Eyal, Lital, Dalia, Noam, Assaf (all report to Eco) and interim manager for Gal + Shir (until Ido live). Owner is manager for Luci + Erez -- Eco facilitates, owner runs or approves test scenarios. For each agent: competency spec (domain requirements + 3 scenarios + pass criteria) -> test execution -> Anat review -> Rambo scan -> Stage C package. Process: company/processes/agent-hiring.md + /hiring command. | P1 | before any go-live A1 | Owner-required 2026-06-14; Eco owns completion; owner involved for Luci + Erez |
| T-0020 | Eco | open | Release notes process: on every PR merge to master, append entry to company/releases/CHANGELOG.md before closing the PR task. Entry must cover: what changed, agents affected (live), agents added (draft), open items. Eco reads CHANGELOG before asserting what any release brought. | P2 | recurring | Created 2026-06-14; first entry written for PR #1 |
| T-0021 | Ido | open | Validate and lock the auto-pull sync mechanism -- three-agent review required before locking: (A) Rambo (Security): review the UserPromptSubmit hook for security risk -- does auto-executing git pull on every session introduce any attack surface? any safer alternative? flag concerns to Eco. (B) Dalia (Q&G): governance review -- is this the right process for keeping agents in sync with master? any quality or audit concerns? alternatives? flag to Eco. (C) Ido (VP R&D, lead): (1) confirm hook fires and pulls master correctly; (2) assess whether 2h wake-up also needs a pull trigger; (3) evaluate edge cases (conflicts, network failure, ff-only rejection) and harden if needed; (4) evaluate whether a better architectural solution exists for the root cause (local clone always out of sync with cloud pushes); (5) incorporate Rambo + Dalia findings; (6) produce final recommendation to Eco -- lock current mechanism or propose improvement. Document outcome in company/processes/. | P1 | urgent; each reviewer acts on activation | Hook firing confirmed 2026-06-15 (PR #2 session); --ff-only error on feature branches is expected + harmless (|| true eats it); on Windows/master it works correctly. Suggested hardened command: `git fetch origin 2>&1 | tail -1; [ "$(git branch --show-current)" = "master" ] && git merge --ff-only origin/master 2>&1 | tail -1 || true`. Owner must apply settings.json edit (classifier blocks agent write). Local->cloud direction also in scope (Eco's local work was never pushed -- see exoneration entry 2026-06-15). |
| T-0022 | Eco | open | Daily summary delivery via email: once Eco has access to a company email account, send the full daily summary to jecki@gmail.com (or owner-designated address) by email. Send via Telegram/chat ONLY for blockers or critical issues -- not the full summary. File save to reports/daily-summaries/ continues regardless of delivery channel. | P2 | when email access available | Owner-requested 2026-06-15 (Eco local work); replaces Telegram as primary summary channel |
