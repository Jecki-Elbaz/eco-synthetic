...
| T-0012 | Dalia | blocked | Access-matrix reconciliation (scope expanded 2026-06-14): formalize .claude/agents/ read-by-exception for BOTH Anat (HR) and Rambo (Security) in company/governance/access-matrix.md. Both exceptions already applied and noted in matrix as bootstrapped by owner A1; Dalia to confirm no gaps, raise concerns, and log as proper A2 decision with Rambo review. | P3 | when Dalia built | Originally Anat-only; expanded to include Rambo (2026-06-14 go-live). Do not act before Dalia exists. |
...
| T-0015 | Eco | in-progress | Parallel P1 agent build: draft role files for ALL pending P1 agents simultaneously -- Ido, Eyal, Lital, Dalia, Noam, Assaf, Gal, Shir, Luci, Erez. All drafts to reach owner-review state together. Owner reviews and approves all at once (A1 per agent). Anat certifies each after A1. Owner-approved 2026-06-14. | P1 | immediate | Blocking company function: agents without role files cannot be reliably invoked. Run in parallel, not sequential. |
