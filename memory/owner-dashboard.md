...
- ~~ONE DECISION, THREE GATES (Anthropic API account for the DPA)~~ -- **DECIDED 2026-08-02 (owner A1): NOT REQUIRED.** The DPA/commercial-plan requirement does not apply to Shelly or internal eco-synthetic activity (owner dev environment; runs under the Max subscription). No API account, no spend. The item re-arms ONLY when a real project goes live, at scale, with real customer data through the LLM. Effect: T-0047 unblocked (proceeds under GR-014); GR-021 DPA condition lifted BUT still blocked on Rambo's security FLAG (hold-window) -- autonomous send is NOT greenlit. Recorded in decisions-log + compliance-backlog ITEM-6.
