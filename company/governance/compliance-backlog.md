# Eco-Synthetic: Compliance Readiness Backlog

Owned jointly by Eyal (Legal) and Lital (CFO). Surfaced to Eco proactively so it is
prioritized on the roadmap and never appears as a sudden critical issue.
Source: constitution section 13. Populated as T-0008.

Status (2026-06-12): initial stub. Eyal and Lital to fill in detail.

---

## Israeli company registration

- Status: not started
- Required by: first customer or first contract
- Owner: Eyal (Legal) + jecki (A1 execution)
- Risk if delayed: cannot legally sign contracts or issue invoices
- Eyal action: confirm timeline, cost, registration type (Ltd / etc.)

## Tax-compliant invoicing

- Status: not started
- Required by: first paid customer
- Owner: Lital (CFO) + Eyal (Legal)
- Notes: Israeli VAT (Maam) registration; Lital owns invoice template
- Lital action: confirm invoicing setup requirements and VAT threshold

## Privacy and data protection

- Status: baseline only (no customer data collected yet)
- Required by: any customer data collected or processed
- Owner: Eyal (Legal)
- Applicable law: Israeli Privacy Protection Law; GDPR if EU customers
- Eyal action: draft data-processing agreement template; identify required disclosures

## ISO readiness

- Status: not started
- Required by: customer or regulatory request (not now)
- Owner: Dalia (Quality + Governance) + Rambo (Security)
- Notes: Adopt ISO 9001 / 27001 as guidance now; certify only if required (constitution section 13)

## Company Google account migration

- Status: pending -- account created (eco.synthetic.org@gmail.com, 2026-06-12); migration blocked on domain approval
- Required by: before first external email or client contact
- Owner: Eco (coordination + execution; was Shelly via S-0002, but Shelly separated 2026-06-20 -- now an external customer, see company/customers/shelly/profile.md -- so reassign execution via Eco/owner)
- Notes: When domain is approved and purchased, migrate eco.synthetic.org@gmail.com to eco@eco-synthetic.[tld]; update all MCP connector auth and the gate-register.
- Action: Eco tracks domain purchase (formerly S-0002) and flags this item when domain is confirmed.

## Open items (Eyal + Lital to resolve)

1. Eyal: confirm Israeli company registration timeline and cost; flag to Eco 30 days before first contract.
2. Lital: confirm invoicing setup requirements (VAT, invoice number series, bank account).
3. Eyal: draft data-processing agreement template before first customer data is collected.
4. Eyal + Lital: agree review cadence for this backlog and report to Eco quarterly.
