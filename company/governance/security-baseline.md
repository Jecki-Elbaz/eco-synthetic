...
| 2026-07-12 | APS hosted demo surface -- external exposure gate (Adam demo, GR-016) | CLEAR-WITH-CONDITIONS | Demo surface assessed against APS-004 pilot gate. Verdict: CLEAR-WITH-CONDITIONS, 10 conditions (DEMO-C1 through DEMO-C10), none blocking the planning/build phase. Risk delta: 16 of 21 pilot must-fix items eliminated (no real LLM, no real student data, no student invite emails). All 6 Eyal-only legal/DPA items eliminated. No Eyal review required. Prompt-injection risk LOW (StubProvider only). HARD condition: DEMO-C4 synthetic-data verification + Rambo counter-sign before any go-live. Owner A1 required for actual go-live. Full conditions: gate-register GR-016. Full findings: Rambo message to Eco 2026-07-12. |
| 2026-07-20 | Weekly permission-drift scan (runner, automated) | ESCALATE_TO_ECO_BLOCKING | 32 role files, gate-register, roster v2.2, spawn-allowlist, .mcp.json, guard.py scanned. BF-1 (5 agents off roster: Oracle, Yael, RedTeam, MeetingPrep, Yossi) + BF-2 (3 stale roster names: Tim/Sally, Noam/Perry, Avner/Jack) -- SIXTH consecutive scan, 14 days overdue. 8 review items open (R-1, R-3/AUD-009, R-4, R-5, R-8, R-9, R-11, R-12). GR-009 addendum: .mcp.json pin + guard.py google-boundary CONFIRMED; credential-store directory NOT CHECKED (runner path limitation R-9). No new ungated tools; no new role files changed; Anat remains only Agent-tool holder. Full report: company/security/reports/permission-drift-2026-07-20.md. |
| 2026-08-02 | BF-1 + BF-2 CLOSED (Rambo, owner-dispatched interactive session) | CLEAR | company/roster.md v2.3 rebuilt from the .claude/agents/ ground truth and verified this session: 32 role files, 32 roster rows, retired names Tim/Noam/Avner and the unnamed Designer absent, the five omitted agents (Oracle, Yael, Yossi, RedTeam, MeetingPrep) present. WHY THE FINDING SURVIVED SEVEN SCANS, all three causes now corrected: (1) the scan job's 300s timeout meant every run since 2026-07-18 ended in error_final, so the scan reporting the finding was itself failing (now 900s via PER_JOB_TIMEOUTS); (2) the owner dashboard read the last run DATE rather than the last TERMINAL event, so a job failing every week displayed as OK (dashboard corrected; integrations/task-hygiene/stale_detector.py now checks trigger health by terminal event); (3) the runner path could not dispatch Rambo to remediate, so the finding had no route to a fix (runner dispatch allowlist, owner A1 2026-08-02). STANDING CHANGE, now in Rambo's role file: a finding that recurs is escalated by scan number -- the second repeat is itself the escalation, not a reason to repeat the note. Full report: company/security/reports/runner-spawn-allowlist-review-rambo-2026-08-02.md section F. |
