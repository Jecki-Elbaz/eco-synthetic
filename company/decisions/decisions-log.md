...
## 2026-07-26 -- SEC-0001 pre-flip guard-diff + AUD-006 access-matrix v1.1 + AUD-010 role-file batch APPLIED (owner A1)

- **Author / gate:** Eco executing owner A1 given in-session 2026-07-26 ("A1 approved: guard-diff + access-matrix + AUD-010 batch"). Three specced-but-unapplied A1 artifacts applied together in one interactive session.
- **AUD-009/AUD-013 guard-diff (guard-diff-consolidated-preflip-2026-07-14.md, Rambo):** all 6 changes applied to .claude/hooks/guard.py -- F-S803 noa->OWNER_SPAWN_ONLY; F-S804 oracle+yael->ALLOWED_AGENTS; F-S805 Dalia PATH_SCOPE += company/policies/ + company/post-mortems/ + company/governance/quality-audit-log.md; F-S806 Eyal PATH_SCOPE += decisions-log.md + company/legal/; F-S807 11 certified-live agents -> ALLOWED_AGENTS + PATH_SCOPE (Yossi EXCLUDED pending Rambo B5 + C2/C3; Designer marketing/ EXCLUDED pending AUD-011 activation); F-S814 manage_gmail_filter runner-path hard deny. py_compile clean. company/legal/ created (.gitkeep). GUARD_MODE remains shadow -- this is NOT the enforce flip (B2 + 7 clean days + coverage C1-C4 still required per SEC-0001). Guard-coverage portion of the pre-flip gate CLEARED.
- **AUD-006 access-matrix (access-matrix-revision-draft-2026-07-14.md, Dalia; Eco A2 2026-07-15):** access-matrix.md v1.0 -> v1.1, changes A/C/D/E/F/G applied -- Oracle chronicle broad-read row (write scope verified vs Oracle.md), runner-state.json + agent-runs.jsonl operational rows, Designer marketing gate notes updated for the delivered Rambo AUD-011 scan (2026-07-25 CLEAR-WITH-CONDITIONS), company/policies/ + post-mortems/ + quality-audit-log.md rows, company/legal/ row, CS-group rows (incl. PPL retention), company/research/ row, per-agent projects/ scope notes. Change B (Yossi .claude/agents/) = no change. POL-001 hyphen clarifier excluded per draft rationale.
- **AUD-010 role-file batch (aud-010-role-file-batch-2026-07-14.md, Anat; re-verified 2026-07-25):** 42 items applied across 20 role files under .claude/agents/, 0 mismatches (3 addendum-corrected pairs used the 07-25 verification versions). Eco.md pre-existing verify-before-forward change folded into the same A1 commit (all .claude/agents/ files are A1 red-path).
- **Process note:** the 07-25 "4th reactivation" wave (T-0004/AUD-010/AUD-004/AUD-011) was root-caused to a dispatch gap -- Eco stale-sweep appends board text but no runner job spawns the responsible agents with their backlog; AUD-010 was additionally a FALSE stall (deliverable existed since 2026-07-14). Fixes landed same session: VERIFY-BEFORE-REACTIVATE + 3rd-cycle-escalate rules added to the Eco sweep prompt; Assaf TAIL-RULE added (killed the false "runner offline 26 days" alerts). AUD-013 item (2) Red Team coaching clauses also delivered (Dalia, redteam-coaching-clauses-dalia-2026-07-26.md) -- found matching text already existed in role-template-clauses-2026-07-14.md, never surfaced as the AUD-013 deliverable (same tracking-gap class).
- **Files affected:** .claude/hooks/guard.py, company/governance/access-matrix.md (v1.1), company/legal/.gitkeep, 20 role files under .claude/agents/ + Eco.md, memory/board.md (AUD-009/AUD-006/AUD-010/AUD-013 rows), company/decisions/decisions-log.md (this entry). Applies via owner terminal commit (agents cannot git-commit).

## 2026-07-26 -- CS-0001 customer communication policy APPROVED (owner A1)

- **Author / gate:** Mike (VP CS) recording owner A1. Owner (jecki) granted A1 in-session 2026-07-26 (authorization list = Mike + Jenny). Eco A2 granted 2026-07-08; all pre-A1 dependencies closed before this entry.
- **Decision:** CS-0001 (company/cs/cs-0001-customer-communication-policy.md) is APPROVED (owner A1 2026-07-26) and ARMED at v1.0. Authorized agents at activation: Mike (VP Customer Success) and Jenny (tier-1 CS support) ONLY. Jack and Ella are NOT authorized at this time; added later as volume warrants via a future owner A1. All customer email drafts route through Mike for review before placement in the owner's draft folder. Hard gate remains intact: no customer contact until a product is live (section 2 condition 2) -- no product is live as of 2026-07-26, so no customer contact is authorized.
- **Rationale:** Both pre-A1 dependencies were closed before this decision: (a) Eco A2 review 2026-07-08, items 1-3 resolved; (b) Eyal retention window delivered + folded into section 8 (v0.2-DRAFT 2026-07-25, AUD-012 F-QUAL05 closed); (c) authorization list named by owner in this A1 (item 4). All five section 9 open questions now resolved.
- **Closes:** AUD-004 (Phase 2 F-D19) CS-comms leg CLOSED. T-0037 CS-comms procedure leg CLOSED.
- **Files affected:** company/cs/cs-0001-customer-communication-policy.md (v0.2-DRAFT -> v1.0; section 6.1 authorized-agents list; section 9 item 4 RESOLVED; section 11 item 4 RESOLVED; version history row 1.0 added), memory/board.md (AUD-004 + CS-0001 rows), company/decisions/decisions-log.md (this entry).

## 2026-07-26 -- git-hygiene guards + pytest installed + guard proof suite repaired (owner jecki directing, in-session)

- **Author / gate:** Claude Code working session, owner (jecki) present and approving each step in-session. Advisory input from Shir (DevOps) and Rambo (Security). No A1 claimed beyond the in-session approvals recorded per item below.
- **Decision 1 (c02bbcd commit message):** ACCEPTED AS-IS. Merge commit c02bbcd carries the git editor template folded into its subject line (`git commit --no-edit` does not strip `#` comments under cleanup=default, and the template has no blank line before the comment block, so `%s` folds the whole paragraph). Rewriting it would require `git push --force` to master (red line 3, A1). Owner accepted Option A: leave history intact, prevent recurrence instead. Shir concurred.
- **Decision 2 (watchdog NOT adopted):** DEFERRED to Phase B. `bridge-git-sync.py` could not import at all -- watchdog is imported in a try/except but `AgentWriteHandler` subclasses `FileSystemEventHandler` at module level, so a missing optional dependency was an import-time NameError and the documented polling fallback was unreachable. Fixed by binding `Observer = None` / `FileSystemEventHandler = object` in the ImportError branch (commit 9fc6444). watchdog==4.0.1 was deliberately NOT installed: the daemon is inert and undeployed (PROPOSAL header, runtime notice line 496, git-sync-phaseA-signoff.md line 25) and its gate-register entry is a Phase B prereq. Installing now would pull a Phase B dependency decision past its own gate.
- **Decision 3 (pytest installed):** pytest==9.1.1 installed into integrations/telegram-bridge/.venv (the interpreter the runner uses), pinned with transitive deps in a new root requirements-dev.txt (commit d6eac48). Owner approved installation in-session. pytest was already named as the framework in the global CLAUDE.md and already imported by .claude/hooks/test_guard.py, so this is installation of an already-declared framework, not a new tool category. GATE-REGISTER ENTRY IS OUTSTANDING -- see Open items below.
- **Decision 4 (guard proof suite):** guard.py is CORRECT; the 15 failing tests were STALE. Rambo assessed and I verified empirically. Two causes: (a) the B1 owner-interactive exemption (SEC-0001 2026-07-01) allows red-path writes when agent_type is empty and RUNNER_CONTEXT is unset, and the tests never set _agent_type, so every red-path test silently exercised the owner exemption rather than the sub-agent threat model it claimed to prove; (b) roster drift -- gal/shir/ido/meetingprep joined ALLOWED_AGENTS while still listed as non-allowlisted, noam was renamed perry (F-R01) while still listed as allowlisted, and anat gained a PATH_SCOPE that correctly denies projects/. NO guard.py change. test_guard.py repaired (commit 73f8f1c): 36 passed, was 15 failed / 23 passed.
- **Structural finding (C1 evidence scope):** PATH_SCOPE is evaluated before _is_red, and all 30 allowlisted agents are scoped away from every red path, so NO (allowlisted agent, red path) pair can prove the _is_red rule for a sub-agent write. The suite now proves _is_red via the runner path (RUNNER_CONTEXT=1, no agent_type) where PATH_SCOPE is skipped, and asserts on the reason string so the rule is pinned by name. Sub-agent coverage is a separate test that honestly claims PATH_SCOPE as its blocking rule. Mutation-verified: stubbing _is_red flips deny to allow and the suite catches it; emptying PATH_SCOPE changes the reason and the suite catches that too, with the red-path rule firing as a genuine backstop.
- **C1 clearance status:** valid for the guard LOGIC throughout; the proof suite was not valid evidence while it had never executed. Restored as of 73f8f1c (36 passed, mutation-verified). GUARD_MODE on disk remains "shadow" -- unchanged by this session.
- **Rationale (why any of this):** an interrupted merge was finalized this session; the follow-up hardening surfaced three latent defects that had no owner and no detection -- a bridge that could not import, a security proof suite that had never run, and a fail-closed guard that would have wedged the bridge shut on a phantom rebase.
...
## 2026-07-27 -- Agent Performance dashboard: deterministic runner job + live local server (A1)

- **Author / gate:** jecki (owner, A1 by direction: "wire it into the runner and auto-refresh; I want to view it directly from my browser, dynamic"). Built by Eco.
- **Decision:** Stood up a per-agent performance dashboard as a DETERMINISTIC, ZERO-TOKEN integration (same pattern as Shir's git-hygiene audit), NOT an LLM job. `integrations/dashboard/agent_dashboard.py` reads memory/agent-runs.jsonl + memory/board.md + memory/runner-state.json and computes per-agent 7-day metrics (activations, active-days, models used, output-volume proxy, error rate, tasks done/wip/open, escalations, sends). Two modes: (1) `snapshot` writes dashboards/agent-performance.html; (2) `serve` runs a read-only HTTP server on 127.0.0.1:8787 that RECOMPUTES on every request -- the dynamic browser view (a refresh always shows current data). Template: integrations/dashboard/template.html.
- **Runner wiring (auto-refresh):** runner.py gained run_agent_dashboard() (constant DASH_SCRIPT/DASH_KEY, function mirroring run_git_hygiene, call in main after git-hygiene). Runs every full cycle (~2h) as a plain subprocess -- no tokens, no Bash-in-agent, guard-safe. Keeps dashboards/agent-performance.html current even if the live server is down. Verified: py_compile OK; dry-run fires "Eco:agent-perf-dashboard"; snapshot + serve both produce valid output (health 200, / 200).
- **Live browser view:** owner registers the serve process as a Task Scheduler service via integrations/dashboard/dashboard-install.ps1 (elevated), or just runs `python integrations/dashboard/agent_dashboard.py serve` to view now at http://127.0.0.1:8787.
...
## 2026-07-27 -- CLAUDE.md red line 6a: append-only via Write, never Edit (SEC-0001 B2 interactive leg)

- **Author / gate:** owner jecki, A1 in-session ("land it now"). Drafted + applied by Eco (shown for draft-review before landing).
- **Decision:** Added red line 6a to the project CLAUDE.md: append-only files (decisions-log.md, memory/log.md; plus machine logs log.jsonl / agent-runs.jsonl) must be appended via the Write tool with full current content + new entry, never Edit -- an in-place Edit is denied by the guard's append-only check and hard-blocks once GUARD_MODE=enforce. Also split the access-tier table so memory/log.md is marked Append-only (board.md stays edit-in-place). Numbered "6a" to preserve existing red-line numbers 7-11 (referenced by number across the repo).
- **Why:** closes the INTERACTIVE-session leg of SEC-0001 B2. The runner leg is APPEND_DISCIPLINE in runner.py (Shir, same day); this covers interactive agents (the Eco/Rambo/Mike Edit offenders Shir diagnosed) + the owner's own sessions via CLAUDE.md auto-load, avoiding N separate .claude/agents role-file A1 edits. Not on the critical path for gate GREEN, but needed before the enforce flip is safe for interactive sessions.
- **PRACTICALITY GAP flagged to Shir + Rambo (follow-up):** for LARGE append-only files (decisions-log.md ~2000 lines, memory/log.md ~72KB) a full-content Write-tool call is impractical and carries red-line-6 corruption risk; the safe method is a programmatic append (open mode 'a', EOF-only -- as used for this very entry). BUT the readiness gate's C4 "pure append" check only fires on a Write-TOOL append event, which the safe programmatic method does not generate. Two consequences to resolve: (1) rule 6a's "use the Write tool" is impractical for large files -- needs a documented safe-append helper or wording that blesses EOF-only programmatic append; (2) C4 may not fire reliably before the ~2026-08-03 gate. Routed to Shir/Rambo; Eco owns tracking.
- **Files affected:** CLAUDE.md (red line 6a + access-tier table split), company/decisions/decisions-log.md (this entry, appended EOF-only).


## 2026-07-29 -- Dashboard telemetry: runner duration_ms + real cost/token surfacing (A2)

- **Author / gate:** jecki (owner direction: "add duration_ms and fix the cost-capture"); Eco built. A2 runner-logging change; dashboard is the owner-A1 integration from 2026-07-27.
- **Finding first (verify-before-claim):** cost-capture was NOT broken. The earlier "all null" was the pre-2026-07-18 window; since ECO-CMDLINE-FIX (total_cost_usd + modelUsage) 271/396 recent done-records already carry real cost_usd + tokens. The dashboard simply was not consuming them (it showed "n/a").
- **Runner change (runner.py):** run_job now logs per run -- duration_ms (WALL-CLOCK, always present, even on the JSON-fallback/error path), duration_api_ms (from the CLI envelope), and tokens_total (input + cache_creation + cache_read + output; the bare input_tokens omits the ~56k cache tokens). _parse_json_output extended to return the envelope duration (5-tuple); error_final also logs duration_ms. imports += time.
- **Dashboard change (agent_dashboard.py + template.html):** compute() sums per-agent cost7 / tokens7 / compute_ms7 + company totals; the table gained a Cost ($), Tokens, and Compute (LLM wall-time) column, and the headline tile is now real Cost + tokens. Honesty: compute-time accrues from 2026-07-29 forward (older runs have no duration -> "-"); cost/tokens real since 2026-07-18.
- **First real numbers (7d):** company ~$152.57 / 1.40M tokens; Eco dominates ~$122 (its 2h cycles -- a genuine cost signal). Verified: py_compile OK, parse unit-test OK (cost/duration/tokens_total extracted), snapshot renders with the new columns.
- **Files affected:** integrations/runner/runner.py, integrations/dashboard/agent_dashboard.py + template.html, dashboards/agent-performance.html (regenerated), company/decisions/decisions-log.md (this entry, appended EOF-only per red line 6a).


## 2026-07-29 -- Eco runner cost trim: skip 2h check-in during owner quiet hours (A2)

- **Author / gate:** Eco (A2 cadence tweak per CLAUDE.md "job frequency" NOT-A1 list); owner-directed ("look into trimming Eco's runner cost").
- **Diagnosis (from the new cost telemetry):** Eco is ~80% of runner spend, ~$17/day. The 2h check-in averages ~$1.43/run; per run ~12-20k OUTPUT (reasoning) tokens vs ~21 out_chars final -- i.e. Eco does a full 7-scan read-everything-and-reason pass (board conditions, Shelly handoff read+reply, screened-mail, 72h stale-task sweep with on-disk deliverable checks) every cycle and usually concludes "nothing." ~4-5 of the ~12 daily cycles land in owner quiet hours (22:00-09:00 Israel) where owner_notify DROPS the output -- so those spawns cost ~$1.4 each and can never reach the owner. The pre-spawn cost gate only skipped on zero-open-tasks (almost never true).
- **Fix (applied):** run_job's Eco-2h cost gate now skips the spawn entirely when quiet_hours_active(). Verified: compiles; quiet-window logic correct (00:00 UTC/03:00 IL quiet; 09:00 UTC/12:00 IL not). Overnight urgents still surface via the other jobs (Rambo inbox screen, git-hygiene) that run regardless; the 09:00 cycle catches up on any sweep/handoff work.
...
## 2026-08-02 -- Session close (Eco, owner interactive session)

- SEC-0001 C4 gate design: Rambo verdict OPTION B (designated append-only canary memory/append-canary.md; Python-append credit rejected as hollow). Owner A1 2026-08-02: one-line guard.py APPEND_ONLY addition, applied by Eco in-session, py_compile clean. C4 canary heartbeat wired into Eco 2h check-in prompt (A3; expires on enforce flip). b2_deploy reverted to 2026-07-27T20:19:05Z per Rambo correction (Eco A2) -- real B2 fix landed 2026-07-27 (runner APPEND_DISCIPLINE + red line 6a). Gate GREEN projected ~2026-08-03; enforce flip remains owner A1 on GREEN.
- Owner parallel-session guard.py revision (owner-confirmed A1 2026-08-02: RUNNER_SPAWN_ALLOW rambo/eyal/dalia/anat cap 3 depth-1 act-only, board-write PATH_SCOPE expansion ~22 agents, yossi ALLOWED_AGENTS + PATH_SCOPE, REPO_HANDOFF_PATH secret-scan bypass fix, cycle_id logging): Rambo delta scan CLEAR-WITH-CONDITIONS (company/security/reports/guard-delta-scan-rambo-2026-08-02.md). ONE pre-flip must-fix M1: Yael.md still scopes the file index at the superseded company/governance path; canonical index moved to memory/wiki/file-index.md 2026-08-02 (documentation-standard.md s10). Yael.md 6-line update DRAFTED and shown to owner -- PENDING owner A1. Allowlist doc already synced by owner session.
...
## 2026-08-02 -- Runner-path sub-agent dispatch (owner A1)

- **Author / gate:** jecki (A1, interactive session); Eco applied; Rambo adversarial review filed.
- **Decision:** The scheduled runner, previously hard-denied from spawning any sub-agent, may now dispatch a narrow allowlist -- RUNNER_SPAWN_ALLOW = rambo, eyal, dalia, anat (all verified Bash-free) -- under four hard limits enforced in .claude/hooks/guard.py: allowlist only, depth 1 (no nested spawn), act cycles only, and a per-cycle cap RUNNER_SPAWN_CAP = 3 enforced fail-closed via memory/runner-spawn-count.json keyed on RUNNER_CYCLE_ID. Bash-holders and code-builders (gal, shir, adi, noa, oren) remain owner-session-only; work needing them is queued in memory/dispatch-queue.md and drained at interactive session start (new rule in CLAUDE.md). The Agent/Task tool is granted only to the two Eco jobs that own the stale-sweep (AM brief + 2h check-in).
- **Rationale:** The runner could dispatch nobody, so Eco's 72h stale-sweep could only append REACTIVATED notes into board rows that no agent would ever read. Documented consequence: T-0004 in-progress 50 days with no build on disk across four reactivation cycles; T-0046/T-0049 gate reviews undispatched 11-19 days; four reactivation waves (AUD-010, AUD-011, AUD-013, T-0018) chasing deliverables that already existed. The chronicle named this root cause repeatedly and it was never fixed because the fix was assigned to an agent who could not be dispatched.
- **Alternatives considered:** (1) keep the runner spawn-denied and add only the dispatch queue -- rejected, unattended progress would still wait for an owner session; (2) no mechanism change, make the documents honest about "runner detects, owner dispatches" -- rejected, it accepts the stall; (3) open dispatch to all non-Bash agents -- rejected, too wide for a first step. Oren excluded despite holding no Bash: he is in guard OWNER_SPAWN_ONLY under SEC-0001, and lifting that needs its own owner A1.
- **Security dependency:** Rambo adversarial review, company/security/reports/runner-spawn-allowlist-review-rambo-2026-08-02.md -- no real holes; one fail-open edge (unreadable counter after cap) fixed in commit 50a5962. The critical same-commit fix: OWNER_SPAWN_ONLY previously tested `origin` alone and relied on the now-false invariant "the runner cannot spawn anyone"; it now tests RUNNER_CONTEXT/BRIDGE_CONTEXT explicitly.
- **Files affected:** .claude/hooks/guard.py, integrations/runner/runner.py, .claude/hooks/test_guard.py, memory/dispatch-queue.md, CLAUDE.md, integrations/runner/agent-prompts.md, company/governance/agent-tool-spawn-allowlist.md, company/governance/access-matrix.md. Commits 1c1f0ec, 50a5962. (EOF-append; no existing entries altered.)

## 2026-08-02 -- Constitution v2.4, communication model amendment (owner A1)

- **Author / gate:** jecki (A1, interactive session); Eco applied.
- **Decision:** Constitution section 5 amended. "Across groups: only via the two VPs" corrected to "via the managers or VPs of the groups involved" (four VP-tier managers plus three sub-chains now exist; every role file already routed correctly while citing a clause that no longer described the company). Two clauses added: DISPATCH IS PART OF COMMUNICATION (a routing instruction that reaches no execution path is not communication; the router names the path -- direct dispatch, a dispatch-queue row, or a named agent with a dated board row -- and a task left otherwise is a process miss by the ROUTER, not the recipient) and STATUS IS OWED, NOT REQUESTED (status field on every result envelope plus a dated board note; blocked is a reportable state surfaced same-day naming the blocker; a deadline is the latest finish, not the start; a silent miss is a process miss).
- **Rationale:** The owner's assessment was that inter-agent communication, ownership and cooperation were weaker than designed. The audit found the design sound on paper and unenforced in practice; these clauses move the two failure modes from convention into the constitution.
- **Alternatives considered:** Leaving the rules as behavioral norms in soul.md and role files -- rejected: they already existed there and were not being followed (Luci never invoked, 24/32 role files silent on blocked, the dispatch gap unfixed for weeks). Constitutional text is the level that binds.
- **Files affected:** company/constitution.md (v2.3 -> v2.4), .claude/agents/Eco.md (version ref). Commit f1093be. (EOF-append; no existing entries altered.)

## 2026-08-02 -- Role-file batch: status/blocked protocol, security clauses, targeted fixes (owner A1)

- **Author / gate:** jecki (A1, interactive session); Eco applied; clauses drafted by Dalia 2026-07-14.
- **Decision:** A uniform "Status and blocked protocol" section inserted into all 32 role files. The AUD-013 security clauses finally inserted after 19 days pending A1: F-RT8-01 impersonation defense into Sami; F-RT8-02 urgency red flags and allowlist-vs-subcommand scope into Ido, Gal, Shir, Oren, Noa. Anat.md given the Outputs/handoffs section it never had (a const s9 minimum, in the agent whose certification decisions are the highest-consequence handoffs) and its stale self-description of its own access-matrix grant corrected. Yael.md authorized-tasker list corrected to include Eco and the scheduled runner for her own registered jobs (she was literally required to refuse her own weekly job). Result-envelope status fields added to Rambo.md, Dalia.md, MeetingPrep.md; loop caps added to Hila.md and MeetingPrep.md. Luci checkpoint rule added to Eco.md: every A2+ owner proposal carries a Luci counter-case or a written waiver. Board write access extended in guard PATH_SCOPE to all acting agents; yossi added to ALLOWED_AGENTS + PATH_SCOPE (he was certified-live with write duties but would have broken on the enforce flip).
- **Rationale:** 24 of 32 role files said nothing about being blocked, so blocked was not a reportable state; there was no fleet-wide status cadence; 22 of 32 agents could not write memory/board.md, the only working status channel; and the 2026-08-01 review found Luci had not been invoked for any major decision.
- **Alternatives considered:** Editing the role-file template only and re-propagating -- rejected: the 32 live files needed the change now, and template activation is itself A1. Applied directly to all 32 in this A1 session.
- **Files affected:** all 32 .claude/agents/*.md, .claude/hooks/guard.py, company/governance/access-matrix.md. Commits f1093be, 1c1f0ec. (EOF-append; no existing entries altered.)

## 2026-08-02 -- Red lines 1 and 2 had no working enforcement; both layers fixed (owner A1)

- **Author / gate:** jecki (A1, interactive session); Eco applied; found by live CLI tool probe.
- **Decision:** Enforcement added for the two absolute prohibitions that had none. A live tool probe showed the Claude CLI does not match Write(path) permission rules -- only Edit(path) rules, which cover all file-editing tools. .claude/settings.json enforced red line 1 (never write .env or any credential file) and red line 2 (sources/ read-only) solely with Write(.env), Write(.env.*), Write(sources/*) -- all three always inert -- and the guard covered neither path. The settings.json rules are rewritten in the Edit(path) form the CLI honors, and guard.py now denies these paths first, hard-enforced regardless of GUARD_MODE, with no exemption for anyone including the owner's own session (the owner edits .env by hand, outside Claude). A regression test asserts settings.json contains no Write(path) rule so the inert form fails the daily proof-suite job.
- **Rationale:** Two absolute red lines had two enforcement layers between them and neither did anything. Nothing in the project actually stopped a write to .env or sources/.
- **Alternatives considered:** Fixing only settings.json -- rejected: a single config form is a single point of failure, and the guard is the layer that binds the runner and bridge paths where settings.json permissions do not apply.
- **Files affected:** .claude/hooks/guard.py, .claude/settings.json, .claude/hooks/test_guard.py. Commit d1e8c2d. (EOF-append; no existing entries altered.)

## 2026-08-02 -- Anthropic DPA / commercial plan not required for internal or Shelly activity (owner A1)

- **Author / gate:** jecki (A1, interactive session); Eco applied.
- **Decision:** The Anthropic DPA / commercial-plan requirement (compliance-backlog ITEM-6, C-E4) does NOT apply to Shelly or to any internal eco-synthetic activity. Both are the owner's personal DEV environment and stay there; they run under the Max subscription (local Claude Code CLI). No agent treats the missing DPA as a blocker for internal or Shelly work, and the item is not to be surfaced as an open owner action or as overdue. The requirement RE-ARMS only when a real project goes LIVE, AT SCALE, with REAL CUSTOMER personal data flowing through the LLM -- at which point Eyal re-raises the three commercial-plan steps as a genuine pre-go-live gate. "Dev", "internal", "Shelly", and "experimental" do not meet the re-arm trigger; a production launch handling other people's real data does.
- **Rationale:** Owner directive. The DPA is a data-controller/processor obligation that bites when third-party personal data is processed through an LLM at scale for real purposes. eco-synthetic and Shelly are the owner's own dev environment with no real customer data at scale, so the obligation is not triggered today, and carrying it as a standing blocker was producing false-overdue escalations and a spurious "open an API account" owner ask.
- **Scope limit (explicit):** This re-key concerns the DPA/commercial-plan posture ONLY. It does NOT relax the C-E3 HARD STOP (student names, health data, or clinical case content never enters files or model context) -- that boundary stands on its own. It also does NOT greenlight autonomous send: GR-021 (Shelly autonomous send) remains BLOCKED on Rambo's independent security FLAG (no hold-period / queue-review window before the send job fires). The owner A1 lifted the legal blocker only; the security gate is unchanged.
...
## 2026-08-02 -- Adam demo pivots to a LIVE hosted real-patient app (owner A1)

- **Author / gate:** owner jecki, A1 in interactive session. Eco surfaced the reality and prepped; owner executes the A1 legs.
- **Decision:** The Adam early-August review shifts from a SENT NUMBERS PACKAGE (the 2026-08-02 3-session run + cover email) to a LIVE HOSTED APP Adam self-explores with a REAL patient. Owner chose "hosted + real patient (API key)" and "prep everything; owner executes A1."
- **Decisive finding (Eco, verified on disk, not assumed):** hosted+real is NOT a config flip. No hosted LLM provider exists in the codebase -- packages/engine/src/llm/providers/ holds only stub + claude-code; the Anthropic/OpenAI cases in llm.module.ts are commented-out placeholders; no @anthropic-ai/sdk dependency. claude-code cannot serve a hosted box (it shells to the owner's LOCAL CLI). So a real R&D build is required: pin @anthropic-ai/sdk, implement AnthropicProvider (LLMProvider interface, already proven by ClaudeCodeProvider), wire + gate it.
- **This overrides demo guardrail #2** (LLM_PROVIDER=stub at all times; decisions-log 2026-07-12) -- a new owner A1, now given. StubProvider was verified this session to be structurally incapable of a real demo (constant analyser -> trust delta 0 forever; "[STUB]" patient text every turn).
- **APS-004 does NOT govern this demo:** synthetic data only (per the 2026-07-12 demo-deploy-plan); the pilot infra/registration chain is not triggered. The remaining gates are narrower: Rambo security re-gate (internet-reachable + external user + real-LLM egress + new dep + new provider adoption), Eyal fast legal (real Anthropic API account vs Max; synthetic data so DPA/C-E4 likely N/A -- confirm), Lital cost sizing, and the owner A1 legs (open API account, provision key, deploy, go-live).
- **Faster alternative Eco surfaced (owner to choose):** local run on the MAX plan (claude-code) + secure tunnel gives Adam a real patient NOW with zero build and zero API spend (downside: owner machine must stay up/reachable); build hosted for the pilot.
