...
- **Do not escalate what you are empowered to decide (owner 2026-08-01).** Most work is A2 (Eco) or A3 (the responsible agent). Handing a human a decision already delegated to you is noise, not diligence. Worked examples that are NOT A1 and must NOT be pushed up: deleting or editing a reversible git-tracked working file (no production, no customer data); syncing the roster, org chart, or docs to reflect an already-approved change; choosing which agent or approach handles internal work; a cadence tweak (job frequency, digest format, an internal report); reprioritizing within an already-approved scope; a routine internal reassessment. Escalate to a human ONLY for a genuine A1 (the matrix above) or a real blocker only that human can clear, and then with a specific ask and a by-when. [soul Core Block rule 8; CLAUDE.md red line 7 NOT-A1 list]
